...
function populateDateTimeTab(hours) {
    var dayNames = {
        0: "Sunday",
        1: "Monday",
        2: "Tuesday",
    ...
    var timeRanges = {};
    var hourValues = hours.values;

    for (var i = 0; i < hourValues.length; i++) {
        var row = hourValues[i];
        var dayIdx = getKeyByValue(dayNames, row[0]);

        timeRanges[i] = { "start": hourValues[dayIdx][1], "end": hourValues[dayIdx][2] };
    }
...
function addTakeoutItem(inventory) {
    ...
    var categoryMap = {"appetizer": [], "lunch": [], "dinner": [], "cocktail": []};

    if (inventory.values.length > 0) {
        for (var i = 0; i < inventory.values.length; i++) {
            var row = inventory.values[i];
            var context = { itemName: row[0], itemDescription: row[1], itemPrice: row[2], itemValue: i + 1 };
            var html = template(context);

            categoryMap[row[3].toLowerCase()].push(html);
        }

        populateTakeoutItemsPerCategory(categoryMap["appetizer"], "Appetizers");
        populateTakeoutItemsPerCategory(categoryMap["lunch"], "Lunch");
        populateTakeoutItemsPerCategory(categoryMap["dinner"], "Dinner");
        populateTakeoutItemsPerCategory(categoryMap["cocktail"], "Cocktails");
    }
}

function populateTakeoutItemsPerCategory(items, category) {
    var itemsHeader = $("#itemsHeader");
    itemsHeader.append("<h3>" + category + "</h3>");

    for (var i = 0; i < items.length; i++) {
        itemsHeader.append(items[i]);
    }
}
